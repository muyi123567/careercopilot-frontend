      ...
      <div className="min-h-screen bg-[linear-gradient(168deg,#FAF7F2_0%,#F7F1E8_38%,#F4EDE2_68%,#FAF7F2_100%)]">
        ...
        <main className="mx-auto max-w-3xl px-4 py-10">
          <div className="flex flex-col items-center text-center">
            ...
            <button onClick={() => navigate('/occupations')} className="mt-4 rounded-full bg-brand-600 px-5 py-2 text-xs font-medium text-white hover:bg-brand-700">返回搜索</button>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen bg-[linear-gradient(168deg,#FAF7F2_0%,#F7F1E8_38%,#F4EDE2_68%,#FAF7F2_100%)]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <nav className="mb-6 flex items-center gap-1.5 text-xs text-ink-400">
          <Link to="/occupations" className="hover:text-brand-700">职业搜索</Link>
        ...
        {/* Title */}
        <h1 className="font-display text-2xl font-semibold tracking-tight text-ink-900">{data.title}</h1>
        ...
        {(data.sector || data.education_level) && (
          <div className="mt-4 flex flex-wrap gap-2">
            {data.sector && <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-medium text-brand-700">{data.sector}</span>}
        ...
        <section className="mt-10">
          <h2 className="font-display text-lg font-semibold text-ink-900">技能要求</h2>
          {skills.length === 0 ? (
        ...
        <div className="mt-10 rounded-2xl border border-line bg-surface p-6 text-center">
          ...
          <Link to="/login" className="mt-4 inline-block rounded-full bg-brand-600 px-6 py-2.5 text-sm font-medium text-white shadow-sm transition-all hover:bg-brand-700">
            开始规划
          </Link>
        </div>
      </main>
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-line/60 bg-paper/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
        <Link to="/occupations" className="flex items-center gap-1.5 text-sm text-ink-600 transition-colors hover:text-brand-700">
