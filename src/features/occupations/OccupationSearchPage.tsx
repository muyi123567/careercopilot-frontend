  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(168deg,#FAF7F2_0%,#F7F1E8_38%,#F4EDE2_68%,#FAF7F2_100%)]">
      ...
      <header className="sticky top-0 z-20 border-b border-line/60 bg-paper/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="font-display text-base font-bold text-ink-900">Career<b className="text-brand-700">Copilot</b></span>
          </Link>
          <div className="flex items-center gap-3">
            {isAuthenticated ? (
              <span className="text-xs text-ink-500">{user?.display_name || user?.email || '已登录'}</span>
            ) : (
              <Link to="/login" className="rounded-full bg-brand-600 px-4 py-1.5 text-xs font-medium text-white transition-colors hover:bg-brand-700">登录</Link>
            )}
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        <h1 className="font-display text-2xl font-semibold tracking-tight text-ink-900">探索职业方向</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="search"
              className="w-full rounded-xl border border-line bg-surface py-2.5 pl-10 pr-4 text-sm text-ink-800 placeholder:text-ink-400 focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
              placeholder="输入职业名称，如：产品经理、数据分析师…"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
            />
          </div>
          <button type="submit" disabled={loading || !query.trim()} className="rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-all hover:bg-brand-700 disabled:opacity-40">
            {loading ? '搜索中…' : '搜索'}
          </button>
        </form>
          ...
          <div className="mt-8">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {['产品经理', '数据分析师', '前端工程师', 'UI设计师', '市场营销', '人力资源'].map(tag => (
                <button key={tag} onClick={() => { setQuery(tag); doSearch(tag); }} className="rounded-full border border-line bg-surface px-3.5 py-1.5 text-xs text-ink-600 transition-colors hover:border-brand-400 hover:text-brand-700">
                  {tag}
                </button>
              ))}
            </div>
